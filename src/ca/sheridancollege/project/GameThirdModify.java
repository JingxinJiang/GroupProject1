...
//                System.out.println("Player2 equal cards: " + player2EqualCard);
        ...
        //calculate score
    int[] score={0,0};
    int win1Size=player1winCard.size();
    int win2Size=player2winCard.size();
        System.out.println("win2size is "+win2Size);
        
    if(win1Size==0&&win2Size==0){System.out.println("Win cards can not both 0. Error");}
    else if (win1Size==0){
        score[1]=score[1]+10;
        System.out.println("Player2 wins this round, get 10 points, score is"+score[1]);
            }
    else if (win2Size==0){
        score[0]=score[0]+10;
        System.out.println("Player1 wins this round, get 10 points, score is"+score[0]);                
            }
    else{
        if(win1Size>win2Size){
            score[0]++;
            System.out.println("Player1 wins,Player1's score is "+score[0]);
            
        }else if(win1Size<win2Size){
            score[1]++;
            System.out.println("Player2 wins, Player2's score is "+score[1]);
        }
   ...
   while(player1winCard.size()!=0&&player2winCard.size()!=0) {
        System.out.println("Player1, would you like to continue? 1 continue, 0 stop");
        int cont1=input.nextInt();
        System.out.println("Player2, would you like to continue? 1 continue, 0 stop");
        int cont2=input.nextInt();
        //player1 and player2 both agree continue, send cards
        if(cont1==1&&cont2==1){
            
    int[] cplayer1HoldCard = new int[win1Size];
    int[] cplayer2HoldCard = new int[win2Size];        
    String[] cplayer1cardNameValue=new String[win1Size];
    String[] cplayer2cardNameValue=new String[win2Size];
    for(int k=0;k<win1Size;k++){
        cplayer1HoldCard[k]=player1winCard.get(k);       
        cplayer1cardNameValue[k]=player1winCardNameValue.get(k);   
        System.out.println("cplayer1"+cplayer1HoldCard[k]);
        System.out.println("cplayer1 with Name"+cplayer1cardNameValue[k]);
        }
    for(int k=0;k<win2Size;k++){        
        cplayer2HoldCard[k]=player2winCard.get(k);        
        cplayer2cardNameValue[k]=player2winCardNameValue.get(k);
        System.out.println("cplayer2"+cplayer2HoldCard[k]);
        System.out.println("cplayer2 with Name"+cplayer2cardNameValue[k]);
        }        
        player1winCard.removeAll(player1winCard);
        player2winCard.removeAll(player2winCard);
        player1winCardNameValue.removeAll(player1winCardNameValue);
        player2winCardNameValue.removeAll(player2winCardNameValue);
    
    
        conGame(cplayer1HoldCard,cplayer2HoldCard,cplayer1cardNameValue,cplayer2cardNameValue,player1winCard,player1winCard);
   ...
   /*     
    //delete
        //function call  need change change change
        while(win1Size!=0&&win2Size!=0){
            conGame(cplayer1HoldCard,cplayer2HoldCard,cplayer1cardNameValue,cplayer2cardNameValue);
        }
        */
       ...
       /* //modify  delete
        int player1L=player1winCard.size();
        int player2L=player2winCard.size();
                                                                                //changejjx
        if(player1winCard.size()>player2winCard.size()&&player2winCard.size()!=0){
            score[0]++;
        }else if(player1winCard.size()<player2winCard.size()&&player1winCard.size()!=0){
            score[1]++;
        }
        
      //decide if one already win all cards
      if (player1winCard.size()==0){
                System.out.println("Player2 wins this round, get 10 points");
                score[1]=score[1]+10;}
            else if (player2winCard.size()==0){
                System.out.println("Player1 wins this round, get 10 points");
                score[0]=score[0]+10;
            }else{System.out.println("Win cards can not both 0. Error");}
            System.out.println("Player1 wins cards: " + player1winCard);
            System.out.println("Player2 wins cards: " + player2winCard);
            ...
            System.out.println("Player1's score is "+score[0]);
            System.out.println("Player2's score is "+score[1]);
   ...
   /*     
    //delete     
        //continue game using win cards
    
    System.out.println("Player1, would you like to continue? 1 continue, 0 stop");
    int cont1=input.nextInt();
    System.out.println("Player2, would you like to continue? 1 continue, 0 stop");
    int cont2=input.nextInt();
    //change wincards to player hold cards
...
*/
    ...
    public static void conGame(int[] player1HoldCard, int[] player2HoldCard, String[] player1cardNameValue,String[] player2cardNameValue, ArrayList<Integer> player1winCard,ArrayList<Integer> player2winCard){
        /* test pass success
        for(int k=0;k<cplayer1HoldCard.length;k++){         
        System.out.println("fcplayer1"+cplayer1HoldCard[k]);
        System.out.println("cplayer1 with Name"+cplayer1cardNameValue[k]);
        }
        for(int k=0;k<cplayer2HoldCard.length;k++){         
        System.out.println("cplayer2"+cplayer2HoldCard[k]);
        System.out.println("cplayer2 with Name"+cplayer2cardNameValue[k]);
        }*/
        
        int player1L=player1HoldCard.length;
        int player2L=player2HoldCard.length;
        final int SIZE=player1L>player2L?player2L:player1L;
                    ...
                    do{ 
                       ...
                       //add card 
                        ...
                        player1EqualCardNameValue.add(player1cardNameValue[i+1]);
                            ...
                            //if one have card, the other no card then, have card win
                        System.out.println("out");
                        if(player1L>player2L){                                  //start change1
                            for(int k=0;k<player1EqualCard.size();k++){
                            player1winCard.add(player1EqualCard.get(k));
                            player1winCard.add(player2EqualCard.get(k)); 
                            player1winCardNameValue.add(player1EqualCardNameValue.get(k));
                            ...
                            for(int k=player2L;k<player1L;k++){
                            player1winCard.add(player1HoldCard[k]);                             
                            player1winCardNameValue.add(player1cardNameValue[k]);                                                        
                            }
                            System.out.println("Player1 wins cards");}
                        else if(player1L<player2L){
                                for(int k=0;k<player1EqualCard.size();k++){
                                player2winCard.add(player1EqualCard.get(k));
                                player2winCard.add(player2EqualCard.get(k)); 
                                player2winCardNameValue.add(player1EqualCardNameValue.get(k));
                                ...
                                for(int k=player1L;k<player2L;k++){
                                player2winCard.add(player2HoldCard[k]);                             
                                player2winCardNameValue.add(player2cardNameValue[k]);                                                        
                                }
                            System.out.println("Player2 wins cards");    
                        }else{
                                for(int k=0;k<player1EqualCard.size();k++){
...
//        int[] score={0,0};
//        
//        if(player1winCard.size()>player2winCard.size()){
//            score[0]++;
//        }else if(player1winCard.size()<player2winCard.size()){
//            score[1]++;
//        }
//        System.out.println("Player1 wins cards: " + player1winCard);
//        System.out.println("Player2 wins cards: " + player2winCard);
